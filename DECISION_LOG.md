...
14. **Made LLM judging opt-in.** No API key or invented judge score is needed to reproduce classification/routing metrics.
15. **Required human–judge calibration.** A judge’s score is evidence only after agreement with a human on the same records is measured with weighted kappa.
